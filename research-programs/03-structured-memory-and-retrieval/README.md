...
- G15A's supplied-coordinate primary and conditional mechanism ladders pass,
  ...
  The retained-checkpoint repair route is closed. G15B-T is the fresh pivot:
  its full-view `F` and strict-history `T` transactional arms use explicit
  commit/edit controls and one monolithic state/read path. Phase 0 passes every
  exact clean-SM75 qualification gate with 38,082 matched active parameters,
  5,632 batch-2 FP32 state bytes, exact history causality under current-token
  mutation, `8.94e-8` maximum FP32 logit residual, exact predictions, and
  finite nonzero gradients. The completed prospective Phase-1 screen then
  formally fails both primary `T` and diagnostic-only `T-AUX`. `T` trails
  matched `F` on mean overwrite at every length, its completed-tail commit F1
  fails every seed/task/length, and seed 2381 collapses. `T-AUX` improves mean
  overwrite and timing, but never reaches the frozen `+0.05` margin and seed
  2381 remains below the commit-F1 gate. G15B-T stops before geometry;
  R5/R5-S remain failed historical evidence. G15B-E then tests matched
  full-view product versus logit-additive effective edits. Its exact-SM75
  Phase-1 screen also fails both arms: `A` misses the frozen `+0.02` margin,
  neither arm passes absolute all-seed quality, and `A` fails valid-event-only
  causal sufficiency for two seeds. A residual-delta write law is only a
  prospective next test. G15B-D now passes exact clean-SM75 Phase-0
  implementation qualification for that coupled residual-delta law from clean
  ...
  learning result exists. Generic transport remains identity by default;
...
- [G15B-D Phase-1 ledger](../../SSM-Models/hybrid_memory_v1_4/G15BD_PHASE1_RESULTS.md)

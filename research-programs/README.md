...
| Hybrid Memory v1.4/v1.4.5, GDN2 gate-law work, and Spin-Dirac G15A through G15B | Programme 03 for addressing, erase/write, observability, and retrieval; Programme 06 for trained model comparisons; Programme 04 only for exact Spin/Clifford representation claims | G11/G12 establish bounded causal learning, G13 rejects long-range archive promotion, and G14 is a constructed mechanism screen. G15A-S separately passes signed 28-generator composition transfer under oracle edit timing. G15B then fails its exact-SM75 three-seed commissioned-controller gate: identity is strongest on generic interleaved retrieval but overwrite remains 0.768--0.833, `C` is worse, and `S` is inferior in every MQAR/overwrite/selective mean cell. Near-perfect addresses coexist with about 0.5 overwrite erase recall because collision history is not observable to the token/local-convolution gate. G15B-R0 exactly replays the identity checkpoints but rejects naive tied delta; a structured one-token write continuation must be preserved while erase is anchored to every local write event. G15C/external-only remains blocked. The completed single-seed G16 SM75 shootout gives official fused Mamba-2 the ordinary-compression win at every context (`1.48571` versus v1.4.5 `1.58335` BPRB at L4096); local GDN2 and OLMo lose and every arm fails learned recall. These remain bounded development results, not generic association success, model-family promotion, full triality, or scaling/efficiency. |

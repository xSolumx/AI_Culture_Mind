...
| [03 — Learned structured memory and retrieval systems](03-structured-memory-and-retrieval/README.md) | Direct/delta memories, learned and hierarchical routing, large-slot state, gathered access, fused kernels, and content-addressed hybrid memory | Frozen memory-core results, bounded v1.4.5 causal-learning evidence, a passed G15A supplied-coordinate mechanism cohort, a failed G15A-L autonomous-attribution cohort, and a G15A-F comparator separation that still fails absolute controller-quality gates; no universal update-law winner or robust ordinary-text long-range recall |
...
| Hybrid Memory v1.4/v1.4.5, GDN2 gate-law work, and Spin-Dirac G15A/G15A-L/G15A-F cohorts | Programme 03 for addressing, erase/write, observability, and retrieval; Programme 06 for trained model comparisons; Programme 04 only for exact Spin/Clifford representation claims | G11/G12 establish bounded causal learning, G13 rejects long-range archive promotion, and G14 is a constructed mechanism screen. G15A supports shared vector/positive-Spin lift only on its designed supplied-coordinate task; G15A-L rejects autonomous attribution under cosine scoring. G15A-F repairs full-frame observability: all rank-56 screens pass and `S` beats `I`, `C`, and `S-broken` by at least 0.05 in every seed/length, but its absolute mean, p95, and maximum-error quality gates fail. The next work is chart leakage/convergence diagnosis, then a prospectively frozen balanced-primitive curriculum/block-scalar optimizer, before G15B or natural text. No model promotion follows |

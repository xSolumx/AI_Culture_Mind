...
The independent
[fixed-generator certificate](../../Spin-Space-Research/docs/experiments/SPIN8_MIXED_CLOSURE_SO8_THEOREM.md)
replays the same closure boundary for one explicit eleven-generator union of
octonion left actions, signed Fano automorphisms, and the golden vector pair.
...
The exterior-power continuation found and repaired the first bottleneck;
endpoint, every-prefix, parallel, and register-resident paths now exist. A
separate 28-direction Cayley scan supplies a continuous dense chart with an
eight-scalar streaming cache. The best unresolved continuation is now a
**fused-kernel-and-non-exterior falsification pair**:
...
3. benchmark the continuous Cayley scan in FP32 and a guarded mixed-precision
   path, reporting whether this Turing GPU actually dispatches Tensor-Core
   instructions rather than inferring that from dtype alone;
4. generate the first irreducible $SO(8)$ modules not supplied by exterior
   powers of the vector representation and interval-certify their norms;
5. reject the macro proposal if fused or backward runtime loses, especially in
   the nearly break-even large-batch CPU regime, or a new
   higher weight restores a near-unit radius; and
6. seek a full algebraic expansion theorem only after those falsifiers pass.

"""Freeze scan backends from independent matched-memory tuning processes."""
...
def select(paths: list[Path]) -> dict[str, object]:
    ...
    for row_index, length in enumerate(lengths):
        ...
        for variant in VARIANTS:
            ...
            winner_time = aggregate_medians[winner]
            diagnostics[variant] = {
                ...
                "relative_gap_to_runner_up": (
                    aggregate_medians[runner_up] / winner_time - 1.0
                ),
            }
            chosen[variant] = int(winner) if variant.endswith("chunkwise") else winner
        selections[str(length)] = {
            "slot_backends": {
                variant: str(chosen[variant]) for variant in VARIANTS[:2]
            },
            "delta_chunk_sizes": {
                variant: int(chosen[variant]) for variant in VARIANTS[2:]
            },
            "diagnostics": diagnostics,
        }

    return {
        "experiment": "frozen matched-memory implementation selection",
        "selection_rule": (
            "minimum median of independent-process candidate medians; tuning "
            "problems use the disjoint frozen tuning seed"
        ),
        "inputs": inputs,

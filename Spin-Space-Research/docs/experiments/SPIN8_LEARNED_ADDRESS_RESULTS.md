...
> **Later matched boundary.** The exact permutation-completion result below is
> preserved. The current campaign adds continuous hierarchical routing and
> shows direct and triality slots tie once the router is held fixed; it does
> not convert this relational routing result into a storage-capacity theorem.

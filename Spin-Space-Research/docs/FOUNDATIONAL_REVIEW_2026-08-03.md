...
> **Current-status pointer, 2026-08-10.** This review remains the historical
> foundation audit. Later work completed matched hierarchical retrieval and a
> stable co-moving FLA delta benchmark, while finding no same-router triality
> overwrite-capacity advantage. It does not alter this review's Cl(3) model
> verdict or the open language-model claim. See
...
> **2026-08-10 completion addendum.** A prospective paired-action replication
> on untouched seeds 20--29 now consumes the learned negative action through
> both direct and standard delta memories. Direct/delta parity is exact, the
> shared family remains exact, and the independently fitted family fails the
> held-out view in 10/10. This closes the bypass empirically while preserving
> the narrower interpretation: shared representation prior, not added memory
> capacity or an improved overwrite law. The first strict historical replay
> failed because its metric-only artifact did not retain learned parameters.

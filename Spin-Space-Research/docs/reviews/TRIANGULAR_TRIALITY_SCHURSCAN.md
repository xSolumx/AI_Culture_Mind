...
> **Current status, 2026-08-10.** The learned-address and full-transport
> systems gates described as future work in earlier revisions are complete.
> Direct and triality-coded slots tie when the router is shared; hierarchical
> routing helps both direct and delta memory; and a co-moving compiler runs the
> transported delta recurrence through official FLA chunk/recurrent kernels.
> See the
> [canonical result](../experiments/SPIN8_SPIN9_HIERARCHICAL_MEMORY_RESULTS.md).
...
## Matched empirical and systems gates

The same-router campaign now compares direct slots, triality slots, generic
bilinear memory, and delta memory under frozen routing, state, parameter,
training-token, and seed contracts. Direct and triality rows are equivalent on
ordinary overwrite/recall, so triality receives credit only for cross-view
completion. Coarse block selection improves learned direct and delta retrieval;
hard selection is exact only before adversarial alias corruption destroys
linear separability.

The eager SchurScan engineering result remains the authority for the triangular
intertwiner implementation. Separately, general invertible value transport has
a stable co-moving reduction to official FLA delta kernels. That benchmark
closes the full-transport fused-memory gap, but it does not fuse the triangular
triality binder or measure end-to-end sparse routing. The exact rows remain
mechanism oracles, not a claim that attention has been replaced.
...
Joint blind action and continuous-alias recovery subsequently passed 10/10
frozen seeds. The remaining gate is architectural integration: a local window,
coarse shared router, selected fine blocks, co-moving global delta summary, and
learned output gate compared against strong sparse-attention, delta, and SSM
baselines on selective-copy/MQAR before any language-scaling claim.

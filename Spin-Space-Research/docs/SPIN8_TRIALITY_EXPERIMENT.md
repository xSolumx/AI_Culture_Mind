...
The later matched campaign removes that paragraph's remaining systems-level
ambiguity. When the router is held fixed, direct and triality-coded slots tie
on ordinary overwrite/recall; triality's defensible benefit is cross-view
action/routing completion. Coarse-to-fine block routing improves both direct
and delta memories over dense learned routing, and a stable co-moving compiler
maps full invertible value transport into official FLA chunk/recurrent delta
kernels. The length-4096 RTX 2070 SUPER result is about fivefold faster than
the direct transported reference, with a declared doubled recurrent state.
See
[`SPIN8_SPIN9_HIERARCHICAL_MEMORY_RESULTS.md`](experiments/SPIN8_SPIN9_HIERARCHICAL_MEMORY_RESULTS.md).

The subsequent large-slot campaign replaces the separable eight-key proxy with
64 overlapping semantic keys. Shared three-view routing and coarse-to-fine
retrieval pass all ten frozen seeds. An actual one-kernel gathered-state
recurrence then beats eager dense and eager gathered controls throughout the
tested RTX 2070 SUPER grid. The action frame is supplied and the kernel is
inference-only, so this is neither action discovery nor extra triality capacity.
See
...
The formerly missing direct/delta causal row is now materialized prospectively
on untouched seeds 20--29. Shared actions remain exact at length 2048; the
independent family averages 0.542924 even when forced through the shared router;
and standard delta memory agrees exactly with direct slots on every common
hard-routed stream. All new artifacts retain the learned actions and routers.
The preceding attempt to reproduce the old metric-only fit remains a strict
replay failure rather than being silently reclassified. See
`experiments/TASK_B_DELTA_ACTION_REPLAY_RESULTS.md` and
`experiments/TASK_B_PAIRED_ACTION_REPLICATION_RESULTS.md`.

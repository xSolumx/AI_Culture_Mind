...
> The current memory/scanning frontier is the
> [2026-08-10 Spin(8)/Spin(9) hierarchical-memory report](experiments/SPIN8_SPIN9_HIERARCHICAL_MEMORY_RESULTS.md).
> In particular, old statements below that learned routing or fused transported
> delta execution are a next gate are historical, not current status.

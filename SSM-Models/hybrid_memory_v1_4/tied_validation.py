"""Run the prospectively frozen v1.4.4 G4f tied-address validation."""
...
if __package__:
    ...
    from .learnability_screen import _train_steps, evaluate
    from .model import HybridMemoryLM, parameter_count
    from .successor_screen import _tied_identity_config
else:
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from hybrid_memory_v1_4.identity_validation import (
        CURRICULUM,  # type: ignore[no-redef]
    )
...
PREREGISTRATION = Path(__file__).with_name("G4F_PREREGISTRATION.md")
VALIDATION_SEEDS = (1511, 1523, 1531)
...
def main() -> None:
    ...
    for model_seed in VALIDATION_SEEDS:
        ...
        evaluations = [
            evaluate(
                ...
                batch_size=32,
                batches=16,
                seed_base=model_seed + 1_200_000,
                device=device,
            ),
            evaluate(
                model,
                ...
                batch_size=32,
                batches=16,
                seed_base=model_seed + 1_300_000,
                device=device,
            ),
        ]
        checkpoint = args.checkpoint_dir / f"hybrid_v1_4_4_g4f_seed{model_seed}.pt"
        torch.save(
            {
    ...
    report = {
        "schema_version": 1,
        "claim_status": "validated label-supervised v1.4.4 tied address"
        if passed
        else "failed v1.4.4 tied-address validation",
        "passed": passed,

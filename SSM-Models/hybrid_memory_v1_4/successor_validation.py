"""Run the prospectively frozen v1.4.2 G4d fresh-seed validation."""
...
if __package__:
    ...
    from .successor_screen import _config
else:
    ...
    from hybrid_memory_v1_4.successor_screen import _config  # type: ignore[no-redef]

PREREGISTRATION = Path(__file__).with_name("G4D_PREREGISTRATION.md")
VALIDATION_SEEDS = (1451, 1453, 1459)
...
def main() -> None:
    ...
    for model_seed in VALIDATION_SEEDS:
        ...
        config: HybridMemoryConfig = _config()
        ...
        evaluations = [
            ...
            evaluate(
                ...
                batch_size=32,
                batches=16,
                seed_base=model_seed + 900_000,
                device=device,
            ),
        ]
        checkpoint = args.checkpoint_dir / f"hybrid_v1_4_2_g4d_seed{model_seed}.pt"
        torch.save(
            {
    ...
    report = {
        "schema_version": 1,
        "claim_status": "validated label-supervised v1.4.2 successor"
        if passed
        else "failed v1.4.2 successor validation",
        "passed": passed,
        ...
        "useful_query_labels_per_seed": sum(
            phase.updates * 32 * phase.queries for phase in DEFAULT_CURRICULUM
        ),
        "runs": runs,
        "aggregate": {
            str(length): {

...
$modelsRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$python = Join-Path $modelsRoot '.venv-cuda\Scripts\python.exe'
$harness = Join-Path $modelsRoot 'mechanistic_group_actions.py'
...
foreach ($seed in $Seeds) {
    ...
    & $python $harness `
        ...
        --seed $seed `
        --output (Join-Path $PSScriptRoot "$stem.json") `
        --checkpoint-output (Join-Path $PSScriptRoot "$stem.pt")

"""Paired Shakespeare gate for a Schur-legal multiplicity query router."""
...
def main() -> int:
    ...
    for router in ROUTERS:
        config = replace(base_config, spin_multiplicity_router=router)
        counts[router] = parameter_count(build_model("pure_spin_v1_2", config))
        result = run_one("pure_spin_v1_2", config, train_stream, validation, device)
        result["multiplicity_router"] = router
    ...
    report = {
        "schema_version": 1,
        "claim_scope": (
            "paired internal v1.2 multiplicity-router quality gate; sequential "
            "timers are not an order-balanced speed claim"
        ),

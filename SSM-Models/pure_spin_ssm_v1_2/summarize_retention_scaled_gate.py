"""Audit and summarize the frozen retention-scaled block-coupling gate."""
...
def main() -> int:
    ...
    for path in args.inputs:
        ...
        if payload["stage"] != "retention_block":
            raise ValueError(f"not a retention-block artifact: {path}")
        if payload["variant_order"] != [
            "independent_v1_2",
            "retention_scaled_block",
        ]:
        ...
        rows.append(
            {
                ...
                "retention_scaled_bits_per_byte": candidate["final_bits_per_byte"],
    ...
    report = {
        "schema_version": 1,
        "claim_scope": "frozen retention-scaled block quality decision",

"""Audit and summarize the frozen static isotypic-spectrum quality gate."""
...
def summarize(paths: list[Path]) -> dict[str, object]:
    ...
    for path in paths:
        ...
        if payload["stage"] != "isotypic_spectrum":
            raise ValueError(f"not an isotypic-spectrum artifact: {path}")
        ...
        rows.append(
            {
                ...
                "isotypic_spectrum_bits_per_byte": candidate["final_bits_per_byte"],
    ...
    return {
        "schema_version": 1,
        "claim_scope": "frozen static isotypic-spectrum quality decision",

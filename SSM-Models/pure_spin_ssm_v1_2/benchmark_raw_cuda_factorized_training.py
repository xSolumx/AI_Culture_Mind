"""Benchmark raw CUDA versus Triton coordinate-factorized Spin training."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/raw_cuda_factorized_training_rtx2070s.json"),
    )
    args = parser.parse_args()
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required")
    torch.manual_seed(20_260_821)
    coordinates = (0.02 * torch.randn(
        args.batch, args.length, args.channels, 28, device="cuda"
    )).requires_grad_()
    ...
    def run(backend):
        output = backend(coordinates, generators, scale, drive, initial)
        gradients = torch.autograd.grad(output, differentiable, output_gradient)
        return output, gradients

    triton_result, triton_us = timed(
        lambda: run(
            lambda *values: factorized_coordinate_spin8_scan(
                *values, backend="triton"
            )
        ),
        warmups=10,
    ...
    raw_result, raw_us = timed(
        lambda: run(raw_cuda_coordinate_factorized_scan),
        warmups=10,
        repetitions=args.repetitions,
    )
    ...
    report = {
        "schema_version": 1,
        "claim_scope": "28-coordinate factor recurrence forward+backward; controller excluded",
        "shape": {
            "batch": args.batch,
            "length": args.length,

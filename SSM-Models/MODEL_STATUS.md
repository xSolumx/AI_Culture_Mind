...
| [Pure Exceptional Delta v1.3.2](pure_f4_delta_ssm_v1_3/README.md) | Experimental Albert/exceptional model | Completed 27D ladder, controlled action learning, exact canonical primitive actions, and sparse-event SM75 recurrent/parallel backends | [Sparse v1.3.2 transport](pure_f4_delta_ssm_v1_3/SM75_PRIMITIVE_TRANSPORT_RESULTS.md) passes representative matched training time/memory and 4K-prefill gates. Action overhead is negligible and cache is 1.38% of Mamba-2, but eager one-token decode is 1.70x slower, the clean long-training ladder remains open, and the existing text host loses all three Mamba quality seeds |
| [Hybrid Memory v1.4/v1.4.5](hybrid_memory_v1_4/README.md) | Active research workspace; no root-level promotion | G-series preregistrations, results, and artifacts inside the active directory | Validated small hybrid causal learner and commissioned synthetic memory; G13 rejects long-context archive promotion; G14 is a constructed mechanism task; after the distinct G15A-L/F observation/precision failures and G15A-R repair, G15A-S confirms composition-only signed-dictionary transfer under oracle edit timing. G15B rejects the token/local-convolution commissioned controller; retained-checkpoint repairs do not promote it. G15B-T passes implementation qualification but fails its transactional quality screen. G15B-E also passes implementation qualification but fails matched product/logit-additive learning. G15B-D passes exact clean-SM75 Phase-0 qualification and completes an execution-only Phase-1 smoke; the smoke intentionally does not pass adjudication or promotion eligibility, semantic `D` is not compute/temp-memory matched to `P`, and quality remains unexecuted. Identity transport remains the generic default before any Spin comparison |
...
- G15B completes its prospectively frozen exact-SM75 quality cohort from clean
  ...
  G15B-T now implements that fresh matched architecture and passes its exact
  clean-SM75 Phase-0 qualification: `F/T` each have 38,082 active parameters
  and 5,632 batch-2 FP32 state bytes, causal-history controls are exact under
  current-token intervention, maximum FP32 logit residual is `8.94e-8` with
  exact predictions, and all declared gradients are finite/nonzero. This
  implementation qualification remains valid. The subsequent exact-SM75
  Phase-1 quality cohort completes 30,600 updates and 125,337,600 tokens but
  formally fails both `T` and diagnostic-only `T-AUX`. `T` is below `F` on
  mean overwrite at every length and fails commit F1 throughout; `T-AUX`
...
- G15B-E is the fresh, prospectively frozen identifiability repair. Product and
  ...
  Exact clean SM75 Phase 0 passes contraction, scan/chunk/step/mask,
  prediction, gradient, and provenance gates. The completed matched Phase-1
  cohort then fails both trained arms across 20,400 updates and 83,558,400
  tokens: additive `A` never reaches the `+0.02` mean overwrite margin, neither
  arm passes absolute all-seed quality, and `A` fails causal-use sufficiency.
  All trained numerical boundaries pass. The next residual-delta write law is
  only prospective; no model promotion or geometry follows.
- G15B-D now passes exact clean-SM75 Phase-0 implementation qualification from
  clean commit `549e6d98d0bebc35fad32daa498486fd075aa906`. Product `P` and residual `D`
  each have 22,161 active parameters and 1,408 FP32 state bytes with matched
  projections, optimizer assignment, state, data, and future budget. Direct
  transition/injection/coupling residuals are zero, numerical parity and all
  declared gradients pass, and only prospective Phase-1 constructed training
  is authorized. The subsequent clean exact-SM75 smoke passes all preflight
  and numerical checks, but its four updates and 3,840 tokens per arm
  intentionally retain `passed=false` and `eligible_for_promotion=false`.
  Semantic `D` is not compute- or temporary-memory-matched to `P`; the quality
  cohort remains unexecuted. There is no G15B-D learning, causal-use,
  efficiency, natural-text, geometry, or promotion result.
...
The fresh transactional pivot is controlled by the
[G15B-T protocol](hybrid_memory_v1_4/G15BT_TRANSACTIONAL_DELTA_PROTOCOL_2026-08-26.md),
with passed implementation evidence in the
[Phase-0 result](hybrid_memory_v1_4/G15BT_PHASE0_QUALIFICATION_RESULTS.md) and
[Phase-0 artifact](hybrid_memory_v1_4/artifacts/g15bt_phase0_qualification_sm75_2026-08-26.json),
SHA-256 `0b4683ad3b66f7dc010e03737550873cd695d46ade897f21e094d38f4ece2438`.
The completed negative learning result is in the
...
The completed negative learning evidence is in the
[G15B-E Phase-1 result](hybrid_memory_v1_4/G15BE_PHASE1_RESULTS.md) and
[G15B-E quality artifact](hybrid_memory_v1_4/artifacts/g15be_phase1_quality_sm75_2026-08-26.json),
SHA-256 `2a41ed4694c4b2df473e08e6a62e455a309ffa7106616ffdfe420346eee4b469`.
The coupled residual-delta successor is frozen in the
[G15B-D protocol](hybrid_memory_v1_4/G15BD_RESIDUAL_DELTA_PROTOCOL_2026-08-26.md),
and passes exact clean-SM75 implementation qualification in the
[Phase-0 result](hybrid_memory_v1_4/G15BD_PHASE0_QUALIFICATION_RESULTS.md) and
[artifact](hybrid_memory_v1_4/artifacts/g15bd_phase0_qualification_sm75_2026-08-26.json),
SHA-256 `44a8556b60db7cb8c5e1edc239255dc510b62f03960e4514a49b645e79123921`.
This authorizes only prospective Phase-1 constructed training, not a learning
or model-quality claim. The clean exact-SM75 execution smoke is retained in the
[G15B-D Phase-1 result](hybrid_memory_v1_4/G15BD_PHASE1_RESULTS.md) and
[smoke artifact](hybrid_memory_v1_4/artifacts/g15bd_phase1_smoke_sm75_2026-08-26.json),
SHA-256 `b96e8e0cf936af6c828c5ec643484c677a556c6e9f687bc453a70ab653e6464e`.
It intentionally does not pass adjudication or establish quality/efficiency;
the frozen quality cohort remains unexecuted.

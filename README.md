...
| Hybrid Memory v1.4/v1.4.5 | Active research workspace | Validated small hybrid causal learning and commissioned synthetic memory; G14 supports decoupled edits only on a constructed task. G15A's exact supplied-coordinate cohort supports shared vector/positive-Spin lift, but not a fixed Clifford/negative-spin read. The clean-SM75 G15A-L learned-coordinate cohort fails in fresh seeds 2153/2161/2179: only seed 2161 at L64 meets the absolute `S` thresholds, no row meets the 0.05 comparator gate, and learned `S-broken` reparameterizes to match `S`. Autonomous attribution therefore fails; full-frame multi-query raw-Frobenius observability is next, before G15B or more geometry. No full three-carrier triality, generic association, natural-text, or scaling promotion follows |

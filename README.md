...
| Hybrid Memory v1.4/v1.4.5 | Active research workspace | Validated small hybrid causal learning and commissioned synthetic memory; G14 supports decoupled edits only on a constructed task. G15A supports supplied-coordinate shared vector/positive-Spin lift; G15A-L rejects cosine-map attribution; G15A-F restores full-frame comparator separation but misses absolute precision. G15A-R then passes exact SM75 four-probe chart learning. G15A-S now passes all three fresh seeds for signed 28-generator composition transfer to unseen full-rank frame banks and global center words, with `S` means `6.89e-7`--`1.18e-6` through L1024 and every comparator/broken-2x gate passing. The bounded claim remains composition-only transfer under oracle edit timing—not learned topology, generic association/address/query learning, negative-spin/Clifford utility, language, full triality, or scaling/efficiency. G16 is only a frozen runtime-qualified four-arm SM75 harness and has no training metric yet; G15B learned association remains next |

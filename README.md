...
| Hybrid Memory v1.4/v1.4.5 | Active research workspace | Validated small hybrid causal learning and commissioned synthetic memory; G14 supports decoupled edits only on a constructed task. G15A supports supplied-coordinate shared vector/positive-Spin lift; G15A-L rejects cosine-map attribution; G15A-F restores full-frame comparator separation but misses absolute precision. G15A-R repairs precision, and G15A-S separately passes fresh-seed signed 28-generator transfer to unseen full-rank frame banks and global center words under oracle edit timing. G15B then fails its exact-SM75 three-seed commissioned-controller gate: identity learns MQAR and selective recall well but remains only 0.768--0.833 on overwrite, while `C` is worse and `S` is inferior to identity in every MQAR/overwrite/selective mean cell. G15B-R0 proves the collision label is locally unobservable and rejects naive erase-equals-write delta. G15B-R1 preserves the learned write continuation but rejects erase at every write. G15B-R2 supplies perfect collision timing and the full write tail, yet post-same-key-overwrite recall falls 10.3--12.1 points. The scalar symmetric erase route, G15C/external-only, and fresh event-erase training are blocked; exact replacement of the complete logical write component is next. Spin transport remains specialized to supplied/coherent-frame tasks. In the completed one-seed G16 SM75 development shootout, official fused Mamba-2 wins every context and beats v1.4.5 by `0.09764` BPRB at L4096; local GDN2 and OLMo Hybrid lose, and all four fail learned recall. This is not learned topology, generic-association success, language-family promotion, full triality, or a scaling law. |

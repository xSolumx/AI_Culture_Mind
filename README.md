...
| Hybrid Memory v1.4/v1.4.5 | Active research workspace | Validated small hybrid causal learning and commissioned synthetic memory; G14 supports decoupled edits only on a constructed task. G15A supports supplied-coordinate shared vector/positive-Spin lift; G15A-L rejects cosine-map attribution; G15A-F restores full-frame comparator separation but misses absolute precision. G15A-R then passes exact SM75 development and fresh confirmation: the selected `G-decay/random` recipe gives `S` mean errors about `1.21e-7`--`1.89e-7` through L1024 and every comparator/broken-2x gate passes. Longer fixed-LR/random also qualifies at 0.0155--0.0285, so decay is not proven necessary; block moments and curriculum are unnecessary. The bounded claim is composition-only chart learning under a four-probe oracle frame and oracle edit timing. G15B generic association/address/query learning is next; no language, full-triality, or scaling promotion follows |

...
| Hybrid Memory v1.4/v1.4.5 | Active research workspace | Validated small hybrid causal learning and commissioned synthetic memory; G14 supports decoupled edits only on a constructed task. G15A's exact supplied-coordinate cohort supports shared vector/positive-Spin lift, but not a fixed Clifford/negative-spin read; G15A-L rejects autonomous attribution under its cosine observation map. G15A-F's full-frame repair separates `S` from `I`, `C`, and `S-broken` by at least 0.05 in every seed/length, but fails its absolute mean, p95, and maximum-error quality gates, so controller precision remains unsolved. Chart leakage/convergence diagnosis, then a prospectively frozen balanced-primitive curriculum/block-scalar optimizer, comes before G15B or natural text. No full three-carrier triality, generic association, natural-text, or scaling promotion follows |

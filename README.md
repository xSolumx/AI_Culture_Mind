...
| Hybrid Memory v1.4/v1.4.5 | Active research workspace | Validated small hybrid causal learning and commissioned synthetic memory; G14 supports decoupled edits only on a constructed task; G15A passes its exact SM75 three-seed primary mechanism cohort, with `S` at 1.0 symmetry macro versus `C` at 0.2 and `I`/`I+C` at 0.1 in every seed, while every arm learns the no-sym delayed control through L1024; supplied coordinates and oracle carrier controls limit this to mechanism evidence, with no generic association, natural-text, or scaling promotion |

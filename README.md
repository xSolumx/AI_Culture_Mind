...
**Current repository review:** 2026-08-26
...
| [Pure Exceptional Delta v1.3.2](SSM-Models/pure_f4_delta_ssm_v1_3/README.md) | Experimental | The 27D `G2 -> Spin(7) -> Spin(8) -> Spin(9) -> F4 -> E6(-26)` ladder is retained. Exact sparse E6 passes matched SM75 training time/memory and 4K-prefill gates; action overhead is negligible and cache is 1.38% of Mamba-2, but eager one-token decode is 1.70x slower, the long-training ladder remains open, and the existing text cohort loses all three Mamba quality comparisons |
| Hybrid Memory v1.4/v1.4.5 | Active research workspace | Validated small hybrid causal learning and commissioned synthetic memory; G14 supports decoupled edits only on a constructed task. G15A supports supplied-coordinate shared vector/positive-Spin lift; G15A-L rejects cosine-map attribution; G15A-F restores full-frame comparator separation but misses absolute precision. G15A-R repairs precision, and G15A-S separately passes fresh-seed signed 28-generator transfer to unseen full-rank frame banks and global center words under oracle edit timing. G15B then fails its exact-SM75 three-seed commissioned-controller gate: identity learns MQAR and selective recall well but remains only 0.768--0.833 on overwrite, while `C` is worse and `S` is inferior to identity in every MQAR/overwrite/selective mean cell. G15B-R0 proves the collision label is locally unobservable; R1 and R2 reject scalar symmetric erase even with the learned write continuation and perfect collision timing. G15B-R3 then gives each commissioned key an oracle replaceable component: ordinary overwrite rises 12.2--12.8 points over learned erase and the constructed guard reaches 1.0, but the frozen gate fails on saturated-baseline improvement and FP32 replay tolerance. G15B-R4 resolves that ambiguity prospectively: only the value-plus-tail arms pass; value-only overwrite remains below learned at every length and collapses for seed 2311. Removing shared background does not hurt the passing tail arms, localizing the useful association to the ambiguous `t+1` ownership rather than background. G15B-R5 then isolates the tail source: background-free strict-history replacement passes every frozen performance and bias-separation check, reaching 0.9424--0.9466 mean overwrite and 1.0 constructed-guard accuracy, but the formal exact-SM75 adjudication fails the prospectively frozen replay/runtime numeric gates. R5-S subsequently fails its prospective scaled-logit gate in all 135 source/cells despite exact categorical behavior, tiny BPQ drift, bounded state/read residuals, bit-exact transitions, and exact FP64 algebra. The retained-checkpoint repair route is closed. The fresh G15B-T transactional architecture passes exact clean-SM75 Phase-0 implementation qualification, but its completed three-seed Phase-1 quality cohort rejects the learning law. G15B-E then passes implementation qualification but fails its matched product/logit-additive quality screen. G15B-D passes exact clean-SM75 Phase-0 implementation qualification, then completes a four-update/3,840-token-per-arm Phase-1 smoke with every execution boundary passing but formal adjudication and promotion eligibility intentionally false. Semantic `D` is not compute- or temporary-memory-matched to `P`; the quality cohort remains unexecuted and no learning or efficiency evidence follows. Spin transport remains specialized to supplied/coherent-frame tasks. In the completed one-seed G16 SM75 development shootout, official fused Mamba-2 wins every context and beats v1.4.5 by `0.09764` BPRB at L4096; local GDN2 and OLMo Hybrid lose, and all four fail learned recall. This is not learned topology, generic-association success, language-family promotion, full triality, or a scaling law. |
...
Its newest completed identity-memory step is
...
Phase 0 passes implementation qualification, but the
[`Phase-1 quality result`](SSM-Models/hybrid_memory_v1_4/G15BE_PHASE1_RESULTS.md)
rejects both trained arms. Additive control never reaches its frozen margin
and fails causal-use sufficiency; the next residual-delta write law is only a
prospective recommendation, and Spin transport remains blocked. See the
[`exact artifact`](SSM-Models/hybrid_memory_v1_4/artifacts/g15be_phase1_quality_sm75_2026-08-26.json).

Its next prospective training step is
[`G15B-D`](SSM-Models/hybrid_memory_v1_4/G15BD_RESIDUAL_DELTA_PROTOCOL_2026-08-26.md),
a matched coupled residual-delta law. The implementation and
[`Phase-0 result`](SSM-Models/hybrid_memory_v1_4/G15BD_PHASE0_QUALIFICATION_RESULTS.md)
pass exact clean-SM75 implementation qualification from clean commit
`549e6d98d0bebc35fad32daa498486fd075aa906`. `P/D` are matched at 22,161
active parameters and 1,408 FP32 state bytes, direct transition/injection and
coupling residuals are zero, and all declared gradients pass. See the
[`artifact`](SSM-Models/hybrid_memory_v1_4/artifacts/g15bd_phase0_qualification_sm75_2026-08-26.json),
SHA-256
...
learning result or geometry authorization exists. Its clean exact-SM75
[`Phase-1 smoke`](SSM-Models/hybrid_memory_v1_4/G15BD_PHASE1_RESULTS.md)
subsequently completed all preflight and numerical checks from commit
`644330c61bf894ed8ff379872d97d5c591c8555c`, but intentionally records
`passed=false` and `eligible_for_promotion=false`: four updates and 3,840
tokens per arm are execution/profiling evidence only. Semantic `D` is not
compute- or temporary-memory-matched to `P`; the frozen quality cohort remains
unexecuted. See the
[`smoke artifact`](SSM-Models/hybrid_memory_v1_4/artifacts/g15bd_phase1_smoke_sm75_2026-08-26.json),
SHA-256
